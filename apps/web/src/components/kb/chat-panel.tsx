...
export function KnowledgeBaseChatPanel({ kbId, className }: KnowledgeBaseChatPanelProps) {
    ...
    function finalizeAssistantResponse(fallback = "") {
    ...
    const modelSelector = models.length > 0 ? (
        <div className="relative">
            <select
                className="w-full appearance-none rounded-xl border border-border/40 bg-background px-4 py-2 pr-10 text-sm font-medium text-foreground shadow-sm transition focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/40 hover:border-primary/60 hover:bg-primary/10 dark:bg-slate-950/70 dark:border-border/50"
                value={selectedModel}
        ...
        <input
            className="w-full rounded-xl border border-border/40 bg-background px-4 py-2 text-sm font-medium text-foreground shadow-sm focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/40 dark:bg-slate-950/70 dark:border-border/50"
            value={selectedModel}
            onChange={(event) => setSelectedModel(event.target.value)}
    ...
    return (
        <section className={cn("flex h-full min-h-[540px] flex-col gap-4", className)}>
            <header className="space-y-2">
                <h1 className="text-2xl font-semibold">{t("title")}</h1>
                <p className="text-muted-foreground">{t("description", { id: kbId })}</p>
            </header>

            <div className="rounded-3xl border border-border/40 bg-card p-6 shadow-[0_32px_120px_-60px_rgba(22,29,60,0.25)] backdrop-blur dark:bg-slate-950/70">
                ...
                <div
                    className="mt-4 flex-1 space-y-4 overflow-y-auto rounded-2xl border border-border/40 bg-card/95 p-4 dark:bg-slate-950/60"
                    role="log"
                    aria-live="polite"
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <label className="flex flex-col gap-2 text-sm text-muted-foreground">
                    ...
                    <textarea
                        className="min-h-[140px] resize-y rounded-2xl border border-border/40 bg-background px-4 py-3 text-sm text-foreground shadow-sm transition focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/40 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-slate-950/70 dark:border-border/50"
