...
def list_jobs(
    ...
    *,
    ...
    """Return the most recent jobs filtered by document or knowledge base."""
...
def get_job(settings: AppSettings, job_id: str) -> Optional[Dict[str, Any]]:
    """Return a single job entry by identifier."""
...
def count_active_jobs(settings: AppSettings, *, kb_id: str | None = None) -> int:
    """Count jobs that are currently queued or processing, optionally filtered by KB."""
...
def _serialize_row(row: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize psycopg rows to plain dict values."""
